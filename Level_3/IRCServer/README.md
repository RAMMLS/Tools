...
Is a small and limited IRC server written in Python. Despite its size,
...
No special installation needed: Just clone the repository and execute miniircd:
    ...
    git clone <your_path>    cd <dir_name>
    ./main --help
...
1. Clone the repository and copy the executable file to a directory in PATH:
        ...
        git clone <your_path>        cd <dir_name>
        cp <dir_name> /usr/local/bin  # or some other directory in your PATH

   You can then execute the program like this:

        main --help

2. Install IRC as a package from the [IRC PyPI project].
...
[IRC PyPI project]: https://pypi.org/project/miniircd/
...
motd file or an SSL PEM file, you'll need to put them in the jail as well:
    ...
    cp main.pem motd.txt /var/jail/main

Remember to specify the paths for `--state-dir`, `--channel-log-dir`, `--motd`
and `--ssl-pem-file` from within the jail, e.g.:

    main --state-dir=/ --channel-log-dir=/ --motd=/motd.txt \
        --setuid=nobody --ssl-pem-file=/main.pem --chroot=/var/jail/main
